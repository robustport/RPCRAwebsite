...
This vignette provides [details](#the-rounded-spgmi-factors-data) concerning the rounding used to create the `factorsSPGMIr` data from the `factorsSPGMI` data, and shows that there are only minor differences (on the order of 1%, and typically much less) in `factorsSPGMIr` data values relative to their `factorsSPGMI` counterparts. This suggest that one will see minimal differences in portfolio performance when using `factorsSPGMIr` in place of `factorsSPGMI`. 

Nonetheless, users of the `PCRA-1.3` package who prefer to use the unrounded  `factorsSPGMI` data can do so in a manner described in the document “CRSP Stocks and SPGMI Factors in PCRA” , which is downloadable from CRAN at <https://cran.r-project.org/web/packages/PCRA>, or can switch to the development version.
...
![Figure 1. PCRA package data sets and their corresponding descriptions](vignette-assets/dataNamesPCRA.png)

### The S&P Datasets

Standard and Poor’s has graciously consented to the inclusion of five datasets in the`PCRA`package, one of which (`SP500`) is updated annually as new year-end data on the S&P 500 ® Index becomes publicly available. The other four datasets are historical and are copied from paper copies of the S&P Analysts’ Handbook. All five data sets are of type `data.frame`.

S&P provides investors with a great deal of information on the S&P 500 ® at <https://www.spglobal.com/spdji/en/indices/equity/sp-500/>, and some (but not all) of this information can be found in `SP500`, which has (among other items), the per-share earnings, revenues, and price level of the S&P 500 ® Index from 1925 to 2025. It is updated once a year, typically in April, after S&P’s analysts finalize all the year-end data for the index and release it to the public. In addition, it provides year-end CPI and Nominal GDP levels
sourced from the Federal Reserve’s FRED database at <https://fred.stlouisfed.org/>.
...
The PCRA dataset `ConferenceBoardETI` is a `data.frame` with the values of the Conference Board’s
Employment Trends Index ™ from November 1973 to September 2023. The Employment Trends Index&trad is a proprietary composite of eight employment indicators, and information on it can be found at <https://www.conference-board.org/topics/employment-trends-index>. The Conference Board publishes a number of indicators of the health of the U.S. and global economies, including the Leading Economic Index ® (<https://www.conference-board.org/topics/us-leading-indicators>) and the Total Economy Database ™ (<https://www.conference-board.org/topics/total-economy-database>), but these are made available only to subscribers to their service.
...
<table>
  <thead>
    <tr>
      <th align="center"><code>factorsSPGMI</code></th>
  ...
  <tbody>
    ...
    <tr>
      <td align="left">Rounded to 4 Sig. Digits</td>
      <td align="center">2702</td>
      <td align="right">58.1</td>
    </tr>
    <tr>
      <td align="left">Level 9 'xz' Compressed</td>
...
![Figure 2. Fractional differences in factorsSPGMIr relative to factorsSPGMI for each of the 14 factors.](vignette-assets/factorsRelativeDiff.png)
...
The authors wish to express their gratitude to CRSP® , SPGMI, S&P Dow Jones Indices, CBOE LiveVol LLC, ICE Data Indices LLC and Tradeweb Markets LLC for allowing us to provide these data sets in the PCRA package. Their generosity will greatly enhance students’ understanding of portfolio construction and risk analysis methods through the use of these data sets in reproducing the examples and doing the various computational exercises provided in the book, as well as by solving problems assigned by their course instructors.
