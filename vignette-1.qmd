...
```r
 names(relDiff) <- names(factorsSPGMI) png(file = "Plots/factorsRelativeDiff.png", width = 6, height = 4, units = "in", pointsize = 6, res = 600) bp <- boxplot(relDiff,xlab = NULL, ylab = "Relative Differences", ylim = c(-0.01, +0.01), xaxt = "n", cex.lab = 1.5) tick <- seq_along(bp$names) axis(1, at = tick, labels = FALSE) text(tick, par("usr")[3] - 0.0015, bp$names, srt = 45, xpd = TRUE) dev.off()
