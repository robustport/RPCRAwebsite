...
```{r echo=2, warning=FALSE, results="hide"}
png(file = "vignette-assets/stocksMicroAll.png", width = 4, height = 3, units = "in", pointsize = 6, res = 600) names(stocksMicro)[c(35,36)] <- c("Market", "Risk-Free") tsPlotMP(stocksMicro[, c(1:4,35,36)], scaleType = "free", stripText.cex = .45, axis.cex = 0.4, lwd = 0.5) dev.off()
```
...
```{r echo=2, warning=FALSE, results="hide"}
png(file = "vignette-assets/eKRstocksMonthly.png", width = 5, height = 4, units = "in", pointsize = 8, res = 600) boxplot(eKR, xaxt = "n", ylim = c(-2,12), main = "Excess Kutosis stocksCRSPmonthly", cex.main = 1.5, col = "cyan") axis(1, at=1:6, labels=times, cex.axis = 1.1) abline(h=0, lty = "dotted") dev.off()
```
...
```{r echo=2, warning=FALSE, results="hide", eval=FALSE}
png(file = "vignette-assets/weeklyStockReturns.png", width = 6,height = 5,units = "in",pointsize = 8, res = 600) tsPlotMP(stocksMicroWeekly[, c(1:5,35)], scaleType = "free", stripText.cex = .45, yname = "RETURNS", layout = c(1,6), type = "l", axis.cex = 0.7, lwd = 0.6) dev.off()
```

![Time series of weekly returns for the first 5 of the 34 MicroCap CRSP® stocks, and the CRSP® Market returns](vignette-assets/weeklyStockReturns.png)
