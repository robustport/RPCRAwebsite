...
The [original vignette page](vignette-2.html) remains the default until this version is approved.
:::
...
```{r label = "stocksMicroAll", echo = 2, warning=false, results = FALSE, eval=false}
png(file = "Plots/stocksMicroAll.png", width = 4, height = 3,
...
![Returns of the first 4 CRSP® microcap stocks, the Market, and the Risk-Free rate from 1997 to 2010](vignette-assets/stocksMicroAll.png)
...
```{r label = "eKRstocksMonthly", echo = 2:4, warning=false, results = FALSE, eval=false}
png(file = "Plots/eKRstocksMonthly.png", width = 5, height = 4, units = "in", pointsize = 8, res = 600)
boxplot(eKR, xaxt = "n", ylim = c(-2,12), main = "Excess Kutosis stocksCRSPmonthly",
...
![Boxplots of cross-sections of CRSP® stock returns excess kurtosis (eKR) for 6 contiguous time intervals](vignette-assets/eKRstocksMonthly.png)
...
```{r label = "weeklyStockReturns", echo = 2:4, warning=false, results = FALSE, eval=false}
png(file = "Plots/weeklyStockReturns.png", width = 6,height = 5,units = "in",pointsize = 8, res = 600)
...
We use a methodology similar to both the CRSP® method ( <https://www.crsp.org/indexes/breakpoints-chart/> ) and and the MSCI method to allocate the 294 firms in the `stocksCRSPmonthly` universe into four disjoint groups. To do so, we first sort the universe of stocks in each month by their market capitalization and then separate them into four disjoint groups using capitalization breakpoints.
